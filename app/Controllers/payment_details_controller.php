 ...
 public function add_fixed_order_details(Request $request)
{
    ...
    return response()->json([
        'success' => true,
        'message' => 'Fixed Order Details added successfully',
        'order_id' => $order_id,
    ]);
}
...
//         // Check for existing order
//         $existing_order = order_details_model::where('franchise_id', $request->input('franchise_id'))
//             ->where('timer_id', $request->input('timer_id'))
//             ->where('user_id', $request->input('user_id'))
//             ->whereNull('transaction_id')
//             ->whereDate('date', $current_date)
...
//             // Update existing order
//             for ($i = 0; $i < count($request->menu_category_ids); $i++) {
//                 $existing_order->menu_category_id = $request->menu_category_ids[$i];
//                 $existing_order->menu_category_name = $request->menu_category_names[$i];
//                 $existing_order->menu_quantity = $request->menu_quantities[$i];
//                 $existing_order->total_menu_price = $request->total_menu_prices[$i];
//             }
...
public function update_payment_status(Request $request, $order_id)
{
    // Check if the payment exists and is active
    $exists = DB::table('order_details')
                ->where('order_id', '=', $order_id)
...
public function get_orderdetails(Request $request, $user_id)
{
    ...
    $orderDetails = order_details_model::where('user_id', $user_id)
        ->where('status', 'active')
        ->whereNotNull('payment_status')
        // Uncomment if you decide to filter by date
        // ->whereDate('date', $current_date)
        ->get();
...
public function get_fixed_orderdetails(Request $request , $user_id)
{


    // Assuming 'OrderDetail' is your Eloquent model name
    $orderDetails = fixed_order_details_model::where('user_id', $user_id)
        ->where('status', 'active')
        ->whereNotNull('payment_status')
        // Uncomment if you decide to filter by date
        // ->whereDate('date', $current_date)
...
public function get_user_order(Request $request , $user_id)
{
...
//             // If the order_id is not yet in the results array, initialize it
//             if (!isset($results[$order_id])) {
//                 $results[$order_id] = [];
//             }

//             // Prepare order detail array
//             $result = [
//                 "order_id" => $orderDetail->order_id,
//               // "payment_id" => $request->input('payment_id'), // Assuming this is the same for all order details (otherwise adjust accordingly)
//                 "franchise_id" => $orderDetail->franchise_id,
//                 "franchise" => $orderDetail->franchise,
//                 "user_id" => $orderDetail->user_id,
//                 "name" => $orderDetail->name,
...
//             // Add order detail array to the results array under the order_id key
    ...
    public function get_franchise_order(Request $request, $franchise_id)
{
    ...
    public function get_fixed_franchise_order(Request $request, $franchise_id)
{
    $orderDetails = DB::table('fixed_order_details')
        ->where('franchise_id', '=', $franchise_id)
        ->where('status', '=', 'active')
        ->get();
