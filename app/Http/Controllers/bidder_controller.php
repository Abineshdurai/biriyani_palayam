...
public function create_bidder(Request $request)
{
    ...
    try {
        ...
        if ($existing_bidder) {
            ...
            if (!$bidder) {
                throw new \Exception('Bidder not added successfully');
            }

            // Increment bidder_count in the time_slots table
            $affectedRows = count_model::where('timer_id', $request->input('timer_id'))
                                       ->where('date', $request->input($current_date))
                                       
                                       ->increment('bidder_count');

            if ($affectedRows < 1) {
                throw new \Exception('Failed to update bidder count');
            }
...
// public function get_bidder(Request $request, $timer_id)
// {
//     $bidders_exist = DB::table('bidder')
//         ->where('timer_id', '=', $timer_id)
//         ->exists();

//     if (!$bidders_exist) {
//         $fail['message'] = "No bidder data found for this timer";
//         $fail['success'] = false;
...
//     if ($bidder_count === 0) {
//         $success['message'] = "No bidder data found for this timer";
