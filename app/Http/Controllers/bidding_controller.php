...
public function get_bidding(Request $request, $franchise_id, $timer_id)
{
    ...
    // Retrieving the highest value of current_price for each menu category of the specified franchise, timer, and current date
    $highest_prices = DB::table('bidding')
        ->select('menu_category_id', DB::raw('MAX(current_price) as highest_price'))
        ->where('timer_id', $timer_id)
        ->where('franchise_id', $franchise_id)
        ->where('date', $current_date)
    ...
    // Fetching the name corresponding to the highest price for each menu category
    foreach ($highest_prices as $highest_price) {
        $category_name = DB::table('bidding')
            ->where('menu_category_id', $highest_price->menu_category_id)
            ->where('current_price', $highest_price->highest_price)
            ->where('menu_image', $highest_price->menu_image)

            ->value('name');
        $highest_price->name = $category_name;
    }

    if ($highest_prices->isEmpty()) {
        $success['message'] = "No bidding data found for this franchise, timer, and current date";
        ...
        $success['success'] = true;
        $success['highest_prices'] = $highest_prices;
    }
    ...
    if ($winners->isNotEmpty()) {
        ...
        $winners = $winners->map(function ($bid) use ($current_price, $current_date) {
            ...
            if (!empty($bid->menu_image)) {
                $image_url = '../../..//uploads/images/menu_images/' . $bid->menu_image;
            }
            return [
                'status' => $bid->status,
                'date' => $current_date,
